        ...
        public Task Run()
        {
            Host = new WebHostBuilder()
                .UseKestrel(options =>
                {
                    ...
                    options.Limits.MaxConcurrentUpgradedConnections = 100; // WebSocket等升级连接数
                    options.Limits.MaxRequestBodySize = 30_000_000; // 30MB请求体限制
                    options.Limits.KeepAliveTimeout = TimeSpan.FromMinutes(2); // Keep-Alive超时
                ...
                .Configure(app =>
                {
                    ...
                    app.Use(async (ctx, next) =>
                    {
                        ...
                        var requestId = PerformanceMonitor.RecordRequestStart(endpoint);
                        ...
                        try
                        {
                            ...
                            stopwatch.Stop();
                            PerformanceMonitor.RecordRequestEnd(endpoint, requestId, stopwatch.ElapsedMilliseconds);
                        }
                        catch (Exception)
                        {
                            stopwatch.Stop();
                            PerformanceMonitor.RecordRequestEnd(endpoint, requestId, stopwatch.ElapsedMilliseconds, true);
                            throw;
                        }
                    });
                    ...
                    app.Run(async http =>
                    {
                        ...
                        {
                            await Response.SimpleResponse(response, 404, "Not found.");
                            return;
                        }
                        ...
                        if (handler != null)
                        {
                            try
                            {
                                ...
                                await handler.Handler.Invoke(req, res);
                            }
                            catch (Exception e)
                            {
                                ...
                                if (DebugMode)
                                {
                                    await Response.SimpleResponse(response, 500, "Internal Server Error: " + e.ToString());
                                }
                                else
                                {
                                    await Response.SimpleResponse(response, 500, "Internal Server Error.");
                                }
                            }
                        }
                        else
                        {
                            await Response.SimpleResponse(response, 404, "We try our best but not found.");
                        }
                    });
                })
                .Build();
