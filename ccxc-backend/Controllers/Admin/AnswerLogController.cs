...
namespace ccxc_backend.Controllers.Admin
{
    ...
    public class AnswerLogController : HttpController
    {
        ...
        public async Task GetUserList(Request request, Response response)
        {
            ...
            var uidItems = await userDb.SimpleDb.AsQueryable().Select(it => new UidItem
            {
                uid = it.uid,
                user_name = it.username
            }).OrderBy(it => it.uid).WithCache().ToListAsync();

            var groupDb = DbFactory.Get<UserGroup>();
            var gidItems = await groupDb.SimpleDb.AsQueryable().Select(it => new GidItem
            {
                gid = it.gid,
                group_name = it.groupname
            }).OrderBy(it => it.gid).WithCache().ToListAsync();
